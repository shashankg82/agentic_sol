...
def parse_bullet_context(text: str):
    ...
    # --- Pattern 1: verbose bullet id / title / content ---
    ...
    matches = pattern_verbose.findall(txt)
    for match in matches:
        bid, title, content = [m.strip() for m in match]
        results.append({"id": bid, "title": title, "content": content})

    # --- Pattern 2: simple [[PBxxxx, Title, Content]] ---
    if not results:
        all_brackets = re.findall(r"\[\[(.*?)\]\]", txt, re.DOTALL)
...
def generator_node_with_playbook(
    ...
    if not summaries:
        ...
        for s in summaries:
            approval_status = s.get("human_approval_status", None)
            human_feedback = s.get("human_feedback", "")

            # --- Determine approval symbol ---
            if approval_status is None or approval_status == "":
                approval = "----------"                         # no feedback yet
            ...
            # --- Human feedback text handling ---
            feedback_text = human_feedback if human_feedback not in (None, "", "None") else "----------"

            lines.append(
                f"- **Task ID:** {s['task_id']}  | Iteration: {s.get('iteration', '')}\n"
                f"  • **Task Name:** {s.get('task_name', '')}\n"
                f"  • **User Request:** {s.get('user_request', '')}\n"
                f"  • **Final Answer:** {s.get('final_answer', '')}\n"
                f"  • **Score:** {s['score']}\n"
                f"  • **Human Feedback:** {feedback_text} | {approval}\n"
                )
        memory_summary = "\n".join(lines)
...
def reflector_node(state: "LangGraphState", vdb) -> "LangGraphState":
    ...
    for rec in all_rows:
        ...
        summaries.append({
            ...
            "human_approval_status": human_fb.get("human_approval_status", None),
    ...
    if not summaries:
        ...
        for s in summaries:
            ...
            lines.append(
                f"- **Task ID:** {s.get('task_id', '')} | Iteration: {s.get('iteration', '')}\n"
                f"  • **Task Name:** {s.get('task_name', '')}\n"
                f"  • **Final Answer:** {s.get('generator_final_ans', '')}\n"
                f"  • **Reasoning:** {s.get('reflector_reasoning', '')}\n"
                f"  • **Errors Identified:** {s.get('reflector_error_identification', '')}\n"
                f"  • **Root Cause:** {s.get('reflector_rootcauseanalysis', '')}\n"
                f"  • **Correct Approach:** {s.get('reflector_correctapproach', '')}\n"
                f"  • **Key Insights:** {s.get('reflector_keyinsights', '')}\n"
                f"  • **Approval:** {approval}\n"
            )
...
def curator_node(state: LangGraphState, vdb) -> LangGraphState:
    ...
    if not summaries:
        ...
        for s in summaries:
            ...
            lines.append(
                f"- **Task ID:** {s.get('task_id', '')} | Iteration: {s.get('iteration', '')}\n"
                f"  • **Curator Action:** {s.get('curator_action', '')}\n"
                f"  • **Curator Content:** {s.get('curator_content', '')}\n"
                f"  • **Score:** {s.get('score', '')}\n"
                f"  • **Approval:** {approval}\n"
            )
...
def consolidator_node(state: LangGraphState, vdb) -> LangGraphState:
    ...
    def ensure_counters(meta: Dict[str, Any]):
        ...
        if not isinstance(meta, dict):
            return meta
        meta.setdefault("helpful_count", int(meta.get("helpful_count", 0)))
        meta.setdefault("harmful_count", int(meta.get("harmful_count", 0)))
        return meta
    ...
    try:
        ...
        def find_internal_id(bid_upper: str):
        ...
        # 1️⃣ First: update all existing bullets from reflector tags
        for bid, tag_value in (bullet_tags or {}).items():
            bid_upper = str(bid).strip().upper()
            tag_value_lower = str(tag_value).lower()
            internal_id = find_internal_id(bid_upper)

            if internal_id is not None:
                meta = playbook_meta[internal_id]
                meta = ensure_counters(meta)
                meta = apply_tags(meta, {bid: tag_value_lower})
                vdb.collections["playbook"]["metadata"][internal_id] = meta
                playbook_updated = True
            else:
                print(f"⚠️ Bullet ID {bid_upper} not found in playbook (may be new).")

        # 2️⃣ Then: if Curator says to ADD, handle the new insight
        if action == "add":
            query_text = f"{title}\n{content}".strip()
            if not query_text:
                print("⚠️ Skipping new bullet creation: empty title/content.")
            else:
                # Similarity check first
                try:
                    ...
                    if results:
                        ...
                        if dist < 0.25:
                            ...
                            meta = ensure_counters(meta)
                            meta = apply_tags(meta, {"similar": "helpful"})
                            internal_sim = best_match["internal_id"]
                            vdb.collections["playbook"]["metadata"][internal_sim] = meta
                            playbook_updated = True
                        else:
                            raise StopIteration
                    else:
                    ...
                    # No similar bullet → create a new one
                    try:
...
def human_feedback_node(state: LangGraphState, vdb) -> LangGraphState:
    ...
    print("Enter your feedback (end with 'EOF' on a new line):")
    lines = []
    while True:
        line = input()
        if line.strip().upper() == "EOF":
...
def main():
    ...
    print("Enter your task description (end with 'EOF' on a new line):")
    lines = []
    while True:
        line = input()
        if line.strip().upper() == "EOF":
